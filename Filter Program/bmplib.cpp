...
void show() {
    string command = "\\After_edit\\After_edit.bmp";
    char cwd[PATH_MAX];
    system(strcat(getcwd(cwd, sizeof(cwd)), command.c_str()));
    usleep(200000);
    shows++;
}
void shown(string imageFileName){
    string command = "\\After_edit\\";
...
void showRGBBMP(unsigned char inputImage[][SIZE][RGB]) {
    writeRGBBMP("./After_edit/After_edit.bmp", inputImage);
    show();
}

void showGSBMP(unsigned char inputImage[][SIZE]) {
    writeGSBMP("./After_edit/After_edit.bmp", inputImage);
    show();
}